...
const TrustWorthySection = React.memo(() => {
  ...
  useEffect(() => {
    if (inView) {
      ...
      const clientsInterval = setInterval(() => {
        setClients((prev) => Math.min(prev + 1, projectCompletion));
      }, 10);

      // Animate On Time Completion Percentage
      const completionInterval = setInterval(() => {
        setCompletionPercentage((prev) => Math.min(prev + 1, projectsDone));
      }, 10);
  ...
  return (
    <div className="bg-gradient-to-b text-white from-primary to-primary py-[2rem]">
      <div
        ref={ref}
        className="w-full grid sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-10 lg:gap-5 p-[2rem]"
      >
        <div
          data-aos="fade-up"
        ...
        <div
          ...
          className="w-full flex flex-col items-center justify-center"
        >
          <h1 className="heading-1">{completionPercentage}%</h1>
          <p className="font-medium mt-1 text-xl">On Time Completion</p>
        </div>
      </div>
    </div>
  );
